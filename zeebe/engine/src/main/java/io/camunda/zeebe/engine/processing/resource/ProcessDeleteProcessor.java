...
public class ProcessDeleteProcessor implements TypedRecordProcessor<ProcessRecord> {
  ...
  @Override
  public void processRecord(final TypedRecord<ProcessRecord> command) {
    processNewCommand(command);
  }

  /** Handles a standalone typed {@code PROCESS.DELETE} command. */
  void processNewCommand(final TypedRecord<ProcessRecord> command) {
    final long resourceKey = command.getValue().getKey();
    ...
    doDeleteProcess(process, resourceKey, false, false, null);
  }
