...
dependencies {
  ...
  add(
    "testImplementation",
    versionCatalog.findLibrary("org-junit-jupiter-junit-jupiter-params").get(),
  )
  add("testRuntimeOnly", versionCatalog.findLibrary("org-junit-jupiter-junit-jupiter-engine").get())
  add(
    "testRuntimeOnly",
    versionCatalog.findLibrary("org-junit-platform-junit-platform-launcher").get(),
  )
...
val it by
  tasks.register<Test>("it") {
    ...
    shouldRunAfter(ut)
    include(itPatterns)
    providers.gradleProperty("it.exclude.tests").orNull?.split(",")?.forEach {
      filter.excludeTestsMatching(it.trim())
    }
  }
